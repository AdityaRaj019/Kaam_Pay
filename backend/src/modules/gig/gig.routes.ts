...
const createGigRateLimiter = (req: Request, _res: Response, next: NextFunction) => {
  ...
  if (record.count >= maxRequests) {
    return next(new AppError('Too many gig creation requests. Please try again in 1 minute.', 429));
  }
